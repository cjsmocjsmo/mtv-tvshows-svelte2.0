<script>
  ...
  function loadNextSeason() {
    ...
    const command = `ncisoriginss${currentSeason}`;
    console.log(command);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: NCISOdatas1.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading NCIS Origins seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $NCISOdatas1 as d}
