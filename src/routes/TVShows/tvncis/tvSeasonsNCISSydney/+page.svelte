<script>
  ...
  // Season NCISSdata stores
  let NCISSdatas1 = writable([]);
  let NCISSdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `ncissydneys${currentSeason}`;
    console.log(command);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: NCISSdatas1.set(response); break;
        case 2: NCISSdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading NCIS Sydney seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $NCISSdatas1 as d}
          <button onclick={() => playtvshow(d.TvId)}>{d.Episode}</button>
        {/each}
      </div>
      <h1>Season 2</h1>
      <div class="seaList">
        {#each $NCISSdatas2 as d}
          <button onclick={() => playtvshow(d.TvId)}>{d.Episode}</button>
        {/each}
      </div>
    </div>
