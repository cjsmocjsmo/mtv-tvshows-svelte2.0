<script>
  ...
  // Season STTVdata stores
  let STTVdatas1 = writable([]);
  let STTVdatas2 = writable([]);
  let STTVdatas3 = writable([]);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: STTVdatas1.set(response); break;
        case 2: STTVdatas2.set(response); break;
        case 3: STTVdatas3.set(response); break;
      }
      currentSeason++;
...
<main>
  <BackArrow path="/" />
  <div>
    <h1>Star Trek: The Original Series</h1>
  </div>

  {#if $loading}
    <div class="loading">
      <p>Loading Star Trek: The Original Series seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $STTVdatas1 as d}
