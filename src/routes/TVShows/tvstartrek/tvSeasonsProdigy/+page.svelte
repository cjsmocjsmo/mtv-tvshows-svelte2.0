<script>
  ...
  // Season data stores
  let PRODdatas1 = writable([]);
  let PRODdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `prodigys${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: PRODdatas1.set(response); break;
        case 2: PRODdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  <BackArrow path="/" />
  <div>
    <h1>Star Trek: Prodigy</h1>
  </div>

  {#if $loading}
    <div class="loading">
      <p>Loading Star Trek: Prodigy seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $PRODdatas1 as d}
