<script>
  ...
  // Season data stores
  let DWdatas1 = writable([]);
  let DWdatas2 = writable([]);
  let DWdatas3 = writable([]);
  let DWdatas4 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `darkwindss${currentSeason}`;
    console.log(command);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: DWdatas1.set(response); break;
        case 2: DWdatas2.set(response); break;
        case 3: DWdatas3.set(response); break;
        case 4: DWdatas4.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading Dark Winds seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
