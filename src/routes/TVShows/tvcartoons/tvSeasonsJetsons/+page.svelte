<script>
  ...
  // Season data stores
  let jetdatas1 = writable([]);
  let jetdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `jetsonss${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: jetdatas1.set(response); break;
        case 2: jetdatas2.set(response); break;
      }
      currentSeason++;
