<script>
  ...
  // Season data stores
  let jqdatas1 = writable([]);
  let jqdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `jonnyquests${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: jqdatas1.set(response); break;
        case 2: jqdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading JonnyQuest seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
