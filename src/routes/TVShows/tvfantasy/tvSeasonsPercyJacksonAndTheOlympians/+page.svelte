<script>
  ...
  // Season data stores
  let PJdatas1 = writable([]);
  let PJdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `percyjacksonandtheolympianss${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: PJdatas1.set(response); break;
        case 2: PJdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading Percy Jackson and the Olympians seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
