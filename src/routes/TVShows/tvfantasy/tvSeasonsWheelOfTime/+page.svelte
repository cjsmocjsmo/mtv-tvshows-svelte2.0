<script>
  ...
  // Season data stores
  let WOTdatas1 = writable([]);
  let WOTdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `wheeloftimes${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: WOTdatas1.set(response); break;
        case 2: WOTdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading The Wheel of Time seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
