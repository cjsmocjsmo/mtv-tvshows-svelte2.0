<script>
  ...
  // Season data stores
  let ROPdatas1 = writable([]);
  let ROPdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `ringsofpowers${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: ROPdatas1.set(response); break;
        case 2: ROPdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading The Lord of the Rings: The Rings of Power seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
