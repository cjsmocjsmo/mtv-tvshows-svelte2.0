<script>
  ...
  // Season data stores
  let HODdatas1 = writable([]);
  let HODdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `houseofthedragons${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: HODdatas1.set(response); break;
        case 2: HODdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading House of the Dragon seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
