<script>
  ...
  function loadNextSeason() {
    ...
    const command = `falconwintersoldiers${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: FALdatas1.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading The Falcon and the Winter Soldier seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
