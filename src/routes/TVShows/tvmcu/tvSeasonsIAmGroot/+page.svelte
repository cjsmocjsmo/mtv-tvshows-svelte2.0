<script>
  ...
  // Season data stores
  let IAGdatas1 = writable([]);
  let IAGdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `iamgroots${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: IAGdatas1.set(response); break;
        case 2: IAGdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading I Am Groot seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
