<script>
  ...
  // Season data stores
  let LOKdatas1 = writable([]);
  let LOKdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `lokis${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: LOKdatas1.set(response); break;
        case 2: LOKdatas2.set(response); break;
      }
      currentSeason++;
