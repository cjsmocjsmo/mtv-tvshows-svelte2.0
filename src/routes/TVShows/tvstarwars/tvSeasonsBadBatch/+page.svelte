<script>
	...
	let BBdatas1 = writable([]);
	let BBdatas2 = writable([]);
	let BBdatas3 = writable([]);
	...
	const unsubscribe = wsLastResponse.subscribe((response) => {
		if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
			switch(currentSeason) {
				case 1: BBdatas1.set(response); break;
				case 2: BBdatas2.set(response); break;
				case 3: BBdatas3.set(response); break;
			}
			currentSeason++;
