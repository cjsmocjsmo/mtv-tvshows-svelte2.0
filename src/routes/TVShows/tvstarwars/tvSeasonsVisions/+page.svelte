<script>
  ...
  // Season VISdata stores
  let VISdatas1 = writable([]);
  let VISdatas2 = writable([]);
  let VISdatas3 = writable([]);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: VISdatas1.set(response); break;
        case 2: VISdatas2.set(response); break;
        case 3: VISdatas3.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading Star Wars: Visions seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
