<script>
  ...
  // Season MANDdata stores
  let MANDdatas1 = writable([]);
  let MANDdatas2 = writable([]);
  let MANDdatas3 = writable([]);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: MANDdatas1.set(response); break;
        case 2: MANDdatas2.set(response); break;
        case 3: MANDdatas3.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading The Mandalorian seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $MANDdatas1 as d}
