<script>
  ...
  function loadNextSeason() {
    ...
    const command = `bobbafetts${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: BFdatas1.set(response); break;
      }
      currentSeason++;
...
<main>
  <BackArrow path="/" />
  <div>
    <h1>The Book of Boba Fett</h1>
  </div>

  {#if $loading}
    <div class="loading">
      <p>Loading The Book of Boba Fett seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
