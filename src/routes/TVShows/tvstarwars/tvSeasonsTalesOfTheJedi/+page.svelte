<script>
  ...
  function loadNextSeason() {
    ...
    const command = `talesofthejedis${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: TOTJdatas1.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading Tales of the Jedi seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $TOTJdatas1 as d}
