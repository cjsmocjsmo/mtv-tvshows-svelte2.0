<script>
  ...
  // Season data stores
  let ANDdatas1 = writable([]);
  let ANDdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `andors${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: ANDdatas1.set(response); break;
        case 2: ANDdatas2.set(response); break;
      }
      currentSeason++;
