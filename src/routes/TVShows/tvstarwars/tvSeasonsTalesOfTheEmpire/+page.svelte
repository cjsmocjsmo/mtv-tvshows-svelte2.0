<script>
  ...
  function loadNextSeason() {
    ...
    const command = `talesoftheempires${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: TOTEdatas1.set(response); break;
      }
      currentSeason++;
...
<main>
  <BackArrow path="/" />
  <div>
    <h1>Tales of the Empire</h1>
  </div>

  {#if $loading}
    <div class="loading">
      <p>Loading Tales of the Empire seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $TOTEdatas1 as d}
