<script>
  ...
  // Season data stores
  let SILdatas1 = writable([]);
  let SILdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `silos${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: SILdatas1.set(response); break;
        case 2:SILdatas2.set(response); break;
      }
      currentSeason++;
