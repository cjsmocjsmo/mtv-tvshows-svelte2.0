<script>
  ...
  // Season data stores
  let FOdatas1 = writable([]);
  let FOdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `fallouts${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: FOdatas1.set(response); break;
        case 2: FOdatas2.set(response); break;
      }
      currentSeason++;
