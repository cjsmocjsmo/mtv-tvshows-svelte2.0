<script>
  ...
  // Season data stores
  let RBWdatas1 = writable([]);
  let RBWdatas2 = writable([]);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: RBWdatas1.set(response); break;
        case 2: RBWdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading Raised by Wolves seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
