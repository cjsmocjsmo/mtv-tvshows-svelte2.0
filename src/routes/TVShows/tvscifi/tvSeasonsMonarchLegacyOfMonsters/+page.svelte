<script>
  ...
  // Season data stores
  let MLOMdatas1 = writable([]);
  let MLOMdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `monarchlegacyofmonsterss${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: MLOMdatas1.set(response); break;
        case 2: MLOMdatas2.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading Monarch: Legacy of Monsters seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $MLOMdatas1 as d}
