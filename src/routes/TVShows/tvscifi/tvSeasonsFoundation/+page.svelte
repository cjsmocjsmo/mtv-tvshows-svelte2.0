<script>
  ...
  // Season FOUNDdata stores
  let FOUNDdatas1 = writable([]);
  let FOUNDdatas2 = writable([]);
  let FOUNDdatas3 = writable([]);
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: FOUNDdatas1.set(response); break;
        case 2: FOUNDdatas2.set(response); break;
        case 3: FOUNDdatas3.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading Foundation seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
      <div class="seaList">
        {#each $FOUNDdatas1 as d}
