<script>
  ...
  // Season data stores
  let HALdatas1 = writable([]);
  let HALdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `halos${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: HALdatas1.set(response); break;
        case 2: HALdatas2.set(response); break;
      }
      currentSeason++;
