<script>
  ...
  // Season data stores
  let ACdatas1 = writable([]);
  let ACdatas2 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `alteredcarbons${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: ACdatas1.set(response); break;
        case 2: ACdatas2.set(response); break;
      }
      currentSeason++;
