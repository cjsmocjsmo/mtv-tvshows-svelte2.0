<script>
  ...
  // Season ORVdata stores
  let ORVdatas1 = writable([]);
  let ORVdatas2 = writable([]);
  let ORVdatas3 = writable([]);
  ...
  function loadNextSeason() {
    ...
    const command = `orvilles${currentSeason}`;
  ...
  const unsubscribe = wsLastResponse.subscribe((response) => {
    if (response && Array.isArray(response) && currentSeason <= totalSeasons) {
      switch(currentSeason) {
        case 1: ORVdatas1.set(response); break;
        case 2: ORVdatas2.set(response); break;
        case 3: ORVdatas3.set(response); break;
      }
      currentSeason++;
...
<main>
  ...
  {#if $loading}
    <div class="loading">
      <p>Loading The Orville seasons...</p>
    </div>
  {:else}
    <div>
      <h1>Season 1</h1>
